...
def load_data(processed_features_directory, train_test_ratio):
    ...
    # Split into X and Y train and test and drop columns we don't need
    X_train = train_df.drop(columns=['ts', 'name', 'apnea_event'])
    X_test = test_df.drop(columns=['ts', 'name', 'apnea_event'])
...
if __name__ == "__main__":
    full_data_path = os.path.join(config.path.EEG_ACC_features_labelled, 'EEG_ACC_features_labelled_ApneaThreshold0.3_ArousalThreshold0.3_window10/')
    X_train, X_val, X_test, y_train, y_val, y_test = load_data(full_data_path, 0.75)
    model, history = train_model(X_train, y_train, X_test, y_test)
    evaluate_model(model, history, X_test, y_test, relevant_threshold_value=0.3, relevant_window_size=10)